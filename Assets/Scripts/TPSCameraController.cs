...
/*	public float moveSpeed = 3f;
	...
	public float cameraShifting = 0f;
	...
	public void LateUpdate() {
		...
		Quaternion rotationWithY = Quaternion.Euler(rotationY, rotationX, 0);
		...
		Vector3 newPosition = (transform.position) - (rotationWithY * Vector3.forward * cameraDistance);
		newPosition += cameraTransform.right * cameraShifting;
		newPosition += cameraTransform.up * cameraHeight;

		cameraTransform.position = newPosition;

		cameraTransform.LookAt(transform.position + transform.up * cameraHeight + transform.right * cameraShifting + transform.forward * 5f);// + cameraTransform.forward * 3f);
	}
...
*/
	...
	void LateUpdate () {
		...
		player.rotation = new Quaternion(0, aimRotation.y, 0, aimRotation.w);
